...
if __name__=='__main__':
    ...
    #---------------------------------------------------------------------#
    fp16            = False
    classes_path='D:\\jupyter\\yolo\\VOCdevkit\\VOC2007\\voc_classes.txt'#指向自己训练数据集的类别文本文件
    anchors_path='D:\\jupyter\\yolo\\VOCdevkit\\VOC2007\\yolo_anchors.txt'#指向先验框对应文本文件
    anchors_mask=[[6,7,8],[3,4,5],[0,1,2]]
    model_path='D:\\jupyter\\yolo\\yolo_weights.pth'#模型的预训练权值文件
    input_shape=[416,416]#输入图片调整后的大小，一定要是32的倍数
    pretrained=False 
    ...
    #解冻阶段训练参数，模型主干发生变化
    unfreeze_epoch=300 #   UnFreeze_Epoch          模型总共训练的epoch,SGD需要更长的时间收敛，因此设置较大的UnFreeze_Epoch,Adam可以使用相对较小的UnFreeze_Epoch
    unfreeze_batch_size=8
    
    freeze_train=True #是否进行冻结训练
    
    init_lr=1e-2        #模型的初始化学习率，也是最大学习率
    min_lr=init_lr*0.01
    ...
    save_period=10     #d每10个epoch保存一次权值
    save_dir='log\\'        #权值保存路径
    
    eval_flag=True    #是否在训练时进行评估
    eval_period=10     #多少个epoch评估一次
    
    num_workers=0      # 用于设置是否使用多线程读取数据,开启后会加快数据读取速度，但是会占用更多内存,内存较小的电脑可以设置为2或者0  
    train_annotation_path='2007_train.txt'#获得训练集图片路径和标签
    val_annotation_path='2007_val.txt'#获得验证集图片路径和标签
...
#         device          = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    class_names,num_classes=get_classes(classes_path)  #获取classes，类别总数
    anchors,num_anchors=get_anchors(anchors_path)      #获取锚框和个数
    ...
    if True:
        unfreeze_flag=False
        if freeze_train:#冻结训练阶段，主干特征提取网络参数不更新
        ...
        optimizer.add_param_group({'params':pg1,'weight_decaye':weight_decay})
        optimizer.add_param_group({'params':pg2})
        
        lr_shedule_func=get_lr_scheduler(lr_decay_type,init_lr_fit,min_lr_fit,unfreeze_epoch)
        ...
        gen=DataLoader(train_dataset,shuffle=shuffle,batch_size=batch_size,num_workers=num_workers,pin_memory=True,drop_last=True,
                      collate_fn=yolo_dataset_collate,sampler=train_sampler)
        gen_val=DataLoader(val_dataset,shuffle=shuffle,batch_size=batch_size,num_workers=num_workers,pin_memory=True,drop_last=True,
        ...
        for epoch in range(init_epoch,unfreeze_epoch):
            ...
            if epoch>=freeze_epoch and not unfreeze_flag and freeze_train:
                ...
                min_lr_fit=min(max(batch_size/nbs*min_lr,lr_limit_min*1e-2),lr_limit_max*1e-2)
        #获得学习率下降的公式
                lr_shedule_func=get_lr_schduler(lr_decay_type,init_lr_fit,min_lr_fiti,unfreeze_epoch)
                for param in model.backbone.parameters():
            ...
            set_optimizer_lr(optimizer,lr_shedule_func,epoch)
